...
public final class ChInputStream implements AutoCloseable {
    ...
    public byte readByte() throws IOException {
        int b = in.read();
        if (b < 0) throw new EOFException("Unexpected end of RowBinary stream");
        return (byte) b;
    }
    ...
    public long readULeb128() throws IOException {
        ...
        while (true) {
            ...
            if ((b & 0x80) == 0) break;
            shift += 7;
            if (shift >= 64) throw new IOException("LEB128 varint too long");
        }
        return result;
    }

    /**
     * Reads a String: LEB128 length prefix then UTF-8 bytes.
     */
    ...
    /**
     * Reads exactly {@code n} bytes and returns them as a String,
